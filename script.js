...
const flightsConversion = function (str) {
    ...
    for (const row of rows) {
        ...
        const output = `${info.includes("Delayed") ? "🔴" : ""} ${info
            .slice(1)
            .replace("_", " ")} from ${dest(from)} to ${dest(
            to
        )} (${time.replace(":", "h")})`;
...
console.log(`avg func declaration: ${calcAverageHumanAge(calcAgeData1)}`);
console.log(`avg func declaration: ${calcAverageHumanAge(calcAgeData2)}`);

///////////////////////////////////////
// WORKING WITH ARRAYS CHALLENGE #03 //
///////////////////////////////////////
...
console.log(`avg func arrow: ${calcAverageArrow(calcAgeData1)}`);
console.log(`avg func arrow: ${calcAverageArrow(calcAgeData2)}`);
